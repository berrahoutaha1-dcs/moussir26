...
export default function AddSupplierModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(language === 'ar' ? 'تم إضافة المورد بنجاح' : 'Fournisseur ajouté avec succès');
        onClose();
        setFormData({
          nomEntreprise: '',
          codeSupplier: '',
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  >
                    ...
                    <option value="Alimentaire">{language === 'ar' ? 'غذائي' : 'Alimentaire'}</option>
                    <option value="Textile">{language === 'ar' ? 'نسيج' : 'Textile'}</option>
                    <option value="Électronique">{language === 'ar' ? 'إلكترونيات' : 'Électronique'}</option>
                    <option value="Construction">{language === 'ar' ? 'مواد بناء' : 'Matériaux de construction'}</option>
                    <option value="Service">{language === 'ar' ? 'خدمات' : 'Prestation de service'}</option>
