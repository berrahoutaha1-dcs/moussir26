...
export default function ImportClientModal({ isOpen, onClose, onImportSuccess }) {
    ...
    const handleFile = async (file) => {
        ...
        try {
            ...
            reader.onload = (e) => {
                ...
                // Parse rows
                const parsedData = rows.filter(row => row.length > 0).map(row => {
                    const rowData = {};
                    columns.forEach(col => {
    ...
    return (
        <div className="fixed inset-0 modal-backdrop flex items-center justify-center z-[200] p-4">
            <div className="modal-container w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden bg-white rounded-2xl shadow-2xl border border-gray-100">
                                        ...
                                        <tbody>
                                            {previewData.map((row, idx) => (
                                                <tr key={idx} className="border-b border-gray-100 hover:bg-blue-50/30 transition-colors">
                                                    {columns.map(col => (
                                                        <td key={col} className="p-4 border-r border-gray-100 text-gray-600 font-medium">
                                                            {row[col]}
                                                        </td>
                                                    ))}
                                                </tr>
                    ...
                    <div className="flex gap-3">
                        {step === 2 && (
                            ...
                            >
                                ...
                                {language === 'ar' ? 'تأكيد الاستيراد النهائي' : 'Confirmer l\'importation'}
                            </Button>
                        )}
                    </div>
                </div>
