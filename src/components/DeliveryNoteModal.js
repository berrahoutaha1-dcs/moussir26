...
export default function DeliveryNoteModal({ isOpen, onClose }) {
  ...
  const [noteNumber, setNoteNumber] = useState('BL-' + Date.now().toString().slice(-6));
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('sales.deliveryNote')}</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2">
                {t('sales.noteNumber') || 'Numéro de bon'}
              </label>
        ...
        {/* Product Selection - Simple inline selection for now */}
        {isProductModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
              ...
              {/* Mock products list - replace with actual product selection */}
