...
export default function AddClientModal({ isOpen, onClose, initialData = null }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const ref = formData.reference || generateReference();
      ...
      let result;
      if (isEditMode) {
        result = await apiService.updateClient(initialData.id, clientData);
      } else {
        result = await apiService.createClient(clientData);
      }

      if (result && result.success) {
        toast.success(isEditMode
          ? (language === 'ar' ? 'تم تحديث العميل بنجاح' : 'Client mis à jour avec succès')
          : (language === 'ar' ? 'تم إضافة العميل بنجاح' : 'Client ajouté avec succès')
        );
      ...
      toast.error(language === 'ar' ? 'حدث خطأ أثناء العملية' : 'Erreur lors de l\'opération');
    } finally {
      setIsLoading(false);
    }
  };
