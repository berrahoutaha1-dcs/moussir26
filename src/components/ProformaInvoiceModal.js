...
import { X, Plus, FileText, Printer, Download } from 'lucide-react';
...
export default function ProformaInvoiceModal({ isOpen, onClose }) {
  ...
  const [invoiceNumber, setInvoiceNumber] = useState('PROF-' + Date.now().toString().slice(-6));
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('sales.proforma')}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2">
                {t('sales.invoiceNumber') || 'Numéro de facture proforma'}
              </label>
            ...
            <div>
              <label className="block text-sm font-medium mb-2">
                {t('sales.validUntil') || 'Valable jusqu\'au'}
              </label>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <p className="text-sm text-yellow-800">
              {t('sales.proformaNote') || 'Note: Une facture proforma est un document préliminaire qui n\'a pas de valeur comptable.'}
            </p>
          </div>
